...
runcmd:
  ...
  - [ bash, -c, 'if [[ "${var.cni_cilium}" == "true" ]]; then echo "cilium" > /root/cni; fi >> /root/cloud-init-run.log 2>&1' ]
  - [ bash, -c, 'if [[ "${var.cni_calico}" == "true" ]]; then echo "calico" > /root/cni; fi >> /root/cloud-init-run.log 2>&1' ]
